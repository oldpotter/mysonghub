...
Component({
  ...
  methods: {
    ...
    onClickOKBtn() {
      ...
      this.triggerEvent('pad', {
        event: 'ok',
        chord: this.data.chord,
        chordInfo: this.data.chordInfo
      }, {})
    },
    ...
    onClickRecentItem(event) {
      // util.cs.log('recent chord:',event.currentTarget.dataset.index)
      const index = event.currentTarget.dataset.index
      util.cs.log(this.data.recentItems[index])
      this.setData({
        chord: this.data.recentItems[index].chord
      })
    },
    ...
    move: function(e) {
      ...
      this.data.arrPoints.map((item, i) => {
        // util.cs.log(i)
        return {
          num: Math.abs(item.a - x) * Math.abs(item.b - y),
          i
        }
      }).forEach(item => {
